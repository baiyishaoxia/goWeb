...
//region Remark:获取window操作系统信息 Author:tang
func WindowsInfo() ([]string, []intfInfo, func() []diskusage) {
	host, _ := os.Hostname()
	info := []string{GetStartTime(), GetUserName(), runtime.GOOS, GetSystemVersion(), GetBiosInfo(),
		GetMotherboardInfo(), GetCpuInfo(), GetMemory(), host, time.Now().Format("2006-01-02 15:04:05"),
	}
	network := GetIntfs()
	disk := GetDiskInfo
	//fmt.Printf("开机时长:%s\n", GetStartTime())
