...
func GetLangsNations() *[]BlogLangNations {
...
func (a *BlogLangs) AddLangs() bool {
...
func LangList(page int, limit int, keywords string, nations_id string) (*[]BlogLangs, float64, float64, int) {
	var langs = new([]BlogLangs)
	...
	err1 := *err
	//记录数
	num, err3 := err1.Table("blog_langs").Count()
	if err3 != nil {
...
func GetLangById(id string) *BlogLangs {
	lang := new(BlogLangs)
	has, err := databases.Orm.Where("id = ?", id).Get(lang)
	if err != nil {
		fmt.Println(err.Error())
...
func GetNations() *[]BlogLangNations {
