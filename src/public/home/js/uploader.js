...
=====================================================*/
//添加图片相册
function addImage(targetObj, src,url,thename) {
    var _rand=Math.random();
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" class="path" name="photo['+_rand+'][path]" value="' + src + '" />'
    + '<input type="hidden" class="remark" name="photo['+_rand+'][info]" value="'+thename+'" />'
...
function setRemark(obj) {
    ...
    var hidRemarkObj = parentObj.prevAll(".remark").eq(0); //取得隐藏值
    var d=parent.layer.open({
            type: 1,
            title: '图片描述',
            area: '500px',
            content: '<textarea id="ImageRemark" style="margin:10px 0;font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:90%;margin:20px 5% 0px 5%;height:150px;">' + hidRemarkObj.val() + '</textarea>', //iframe的url
            btn: ['确定', '取消'], //按钮
                btn1:function(){
    ...
    /*var d = parent.dialog({
        ...
        button: [{
            ...
            callback: function () {
                ...
                parentObj.parent().parent().find("li .remark").val(remarkObj.val());
