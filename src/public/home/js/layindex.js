...
function loadMenuTree(_islink) {
    ...
    //发送AJAX请求
	$.get(adminMenuData,function (data) {
					...
					if(typeof(liObj.children("a").attr("href"))!="undefined"){
						...
						liObj.children("a").click(function () {
							...
							if(typeof($(this).attr("navid"))!="undefined"){
								addCookie("ruiec_manage_navigation_cookie", $(this).attr("navid"), 240);
							}
						});
					}
				}
...
function linkMenuTree(islink, navid) {
	...
	if (argument == 2) {
		...
		cookieObj = navGroupObj.find('a[navid="' + getCookie("ruiec_manage_navigation_cookie") + '"]');
	}
	if (cookieObj.length > 0) {
		...
		//检查是否需要保存到cookie
		if (argument == 2) {
			addCookie("ruiec_manage_navigation_cookie", navid, 240);
		}
		//检查是否需要跳转链接
		...
		navGroupObj.find("ul li a").removeClass("selected");
		//保存到cookie
		addCookie("ruiec_manage_navigation_cookie", "", 240);
	}
}
